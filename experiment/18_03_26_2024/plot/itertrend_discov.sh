...
table_name_small="itertrend_discov"
...
for del in 3 4 5
    ...
    for rho in 05 2 45
        ...
        echo "{data_for_figure/sparse_kmeans_isee_denoise_${table_name}_true.csv};" >> ${code_dir}/temp_code
        echo "\addlegendentry{ iter\_ISEE\_rate }" >> ${code_dir}/temp_code
        #
        echo "\addplot[red, dashed, mark options=solid, forget plot]" >> ${code_dir}/temp_code
        echo "table [x=iter, y= ${del}0.${rho}, col sep = comma ]" >> ${code_dir}/temp_code
        echo "{data_for_figure/sparse_kmeans_isee_denoise_${table_name}_false.csv};" >> ${code_dir}/temp_code
        #
        echo "\addplot[blue, solid, mark options=solid]" >> ${code_dir}/temp_code
        echo "table [x=iter, y= ${del}0.${rho}, col sep = comma ]" >> ${code_dir}/temp_code
        echo "{data_for_figure/sparse_kmeans_isee_${table_name}_true.csv};" >> ${code_dir}/temp_code
        echo "\addlegendentry{ iter\_ISEE\_gaussmax }" >> ${code_dir}/temp_code
        #
        echo "\addplot[blue, dashed, mark options=solid, forget plot]" >> ${code_dir}/temp_code
        echo "table [x=iter, y= ${del}0.${rho}, col sep = comma ]" >> ${code_dir}/temp_code
        echo "{data_for_figure/sparse_kmeans_isee_${table_name}_false.csv};" >> ${code_dir}/temp_code
        #
        echo "\addplot[orange, solid, mark options=solid]" >> ${code_dir}/temp_code
        echo "table [x=iter, y= ${del}0.${rho}, col sep = comma ]" >> ${code_dir}/temp_code
        echo "{data_for_figure/sparse_kmeans_glasso_${table_name}_true.csv};" >> ${code_dir}/temp_code
        echo "\addlegendentry{ iter\_glasso }" >> ${code_dir}/temp_code
        #
        echo "\addplot[orange, dashed, mark options=solid, forget plot]" >> ${code_dir}/temp_code
        echo "table [x=iter, y= ${del}0.${rho}, col sep = comma ]" >> ${code_dir}/temp_code
        echo "{data_for_figure/sparse_kmeans_glasso_${table_name}_false.csv};" >> ${code_dir}/temp_code    
        #

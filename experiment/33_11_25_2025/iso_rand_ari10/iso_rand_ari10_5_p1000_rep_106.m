...
model = 'iso';
cluster_1_ratio = 0.5;
sep = 5; % Separation parameter
n = 200;     % Number of samples
p = 1000;     % Number of features/dimensions (current loop variable)
rep = 106; % Repetition number (current loop variable)
delta = 0;
...
[iter_stop, cluster_est]=clusterer.fit_predict(300, 10, 10, 0.1);
acc = get_bicluster_accuracy(cluster_est, label_true);
table = get_database_subtable_rand(rep, sep, 1:10,  p, n,  acc, iter_stop );

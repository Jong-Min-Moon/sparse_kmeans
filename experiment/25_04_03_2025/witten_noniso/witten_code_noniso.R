...
# Simulation loop
  ...
  for (rep in rep_start:rep_end) {
    ...
    data <- generate_sparse_Graph_Gaussian(separation, s, dimension, n, rho, seed)
    x <- data$x
    ...
    results_list[[rep]] <- list(
      rep = rep, sep = separation, dim = dimension, rho = rho, acc = acc
    )

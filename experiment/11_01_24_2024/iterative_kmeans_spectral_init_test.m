function [survive_total, survive_correct, survive_wrong] = iterative_kmeans_spectral_init_test(x, sigma, K, cluster_true)     
%data generation
% created 01/11/2024
n = size(x,1);
p = size(x,2);
...
% iterate
   ...
   % fprintf("%i entries survived \n\n",sum(thresholder_vec))
    entries_survived = find(thresholder_vec);
    survive_correct = sum(entries_survived<=10);
    survive_wrong = sum(entries_survived>10);
    survive_total = survive_correct + survive_wrong;

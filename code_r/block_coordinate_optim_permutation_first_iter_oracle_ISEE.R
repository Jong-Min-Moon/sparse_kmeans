# Permutation-Based Iterative SDP K-Means (FDR Control) — First-Iter Oracle ISEE Variant
# MODIFICATION (sim09): ISEE uses true_labels on iteration 1 only;
# from iteration 2 onward it reverts to estimated cluster labels (identical to sim07).
library(foreach)
...
#' @export
block_coordinate_optim_permutation_first_iter_oracle_ISEE <- function(X, K, n_iter = 50, n_perms = 20, fdr_target = 0.1, stable_iter = 5, true_labels = NULL) {
    ...
    while (!is_stop && iternum < n_iter) {
        ...
        # Experimental variant (sim09):
        # Use true cluster labels only at first iteration;
        # use estimated labels from iteration 2 onward.
        isee_labels <- if (iternum == 1 && !is.null(true_labels)) {
            cat("Running ISEE with TRUE labels (Oracle — iteration 1 only)...\n")
            true_labels
        } else {
            cat("Running ISEE with estimated labels...\n")
            cluster_est_now
        }
        res_isee <- ISEE_residual_lasso(X, isee_labels, K)

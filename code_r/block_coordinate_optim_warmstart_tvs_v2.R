...
block_coordinate_optim_warmstart_tvs <- function(X, K,
    ...
    for (iter_t in 1:n_iter_tvs) {
        ...
        res_cluster <- run_clustering_block_knowncov(X_tilde, S_hat_tvs, K, cluster_est_now, covariance = Sigma_full, max_iter = 4000)
        cluster_est_new <- res_cluster$cluster
    ...
    res_cluster <- run_clustering_block_knowncov(X_tilde, final_selection, K, cluster_est_now, covariance = Sigma_full, max_iter = 4000)

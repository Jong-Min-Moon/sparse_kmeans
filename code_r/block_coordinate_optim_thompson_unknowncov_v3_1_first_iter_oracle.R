...
#' Block Coordinate Optimization with Thompson Sampling (Unknown Covariance, v3_1 First-Iter Oracle ISEE)
#'
#' Implements the block coordinate ascent with Thompson Sampling for feature selection.
#' sim10 variant: true cluster labels are used for ISEE at iteration 1 only.
#' From iteration 2 onward, estimated cluster labels are used (same as v3 baseline).
#'
...
#' @export
block_coordinate_optim_thompson_unknowncov_v3_1_first_iter_oracle <- function(X, K, n_iter = 100, C = 0.5, n_perms = 200, p_val_threshold = 0.01, max_iter_sdp = 4000, true_labels = NULL) {
    if (!is.matrix(X)) stop("X must be a matrix")
    if (is.null(true_labels)) {
        stop("true_labels must be provided for sim10 (First-Iter Oracle ISEE).")
    }
    ...
    for (iternum in 1:n_iter) {
        ...
        # sim10 variant:
        # Use true cluster labels only at iteration 1.
        # From iteration 2 onward, use estimated cluster labels.
        if (iternum == 1) {
            cat("Using TRUE cluster labels for ISEE transformation (Oracle — iteration 1 only).\n")
            cluster_isee_input <- true_labels
        } else {
            cat("Using estimated cluster labels for ISEE transformation.\n")

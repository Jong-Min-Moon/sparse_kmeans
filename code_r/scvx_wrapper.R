...
run_scvx <- function(X, K, seed) {
    ...
    scvx_pipeline <- tryCatch({
        ...
        w_raw  <- scvxclustr::dist_weight(t(X) / sqrt(p), phi = 0.5,
        ...
        for (g1 in g1_grid) {
            for (g2 in g2_grid) {
                ...
                if (!is.null(fit)) {
                    # Extract cluster labels (connectivity tolerance 1e-3)
                    V_mat   <- fit$V[[1]]
                    ...
                    model_results[[key]] <- list(cluster = labels, selected = selected,
                                                 g1 = g1, g2 = g2)

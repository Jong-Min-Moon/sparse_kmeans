...
# Everything else is identical to cluster_greedy_ISEE.R.
library(foreach)
...
#' @export
block_coordinate_optim_permutation_oracle_ISEE <- function(X, K, n_iter = 50, n_perms = 20, fdr_target = 0.1, stable_iter = 5, true_labels = NULL) {
    ...
    while (!is_stop && iternum < n_iter) {
        ...
        # --- ISEE Transformation (ORACLE: true_labels used instead of cluster_est_now) ---
        cat("Running ISEE with TRUE labels (Oracle)...\n")
        res_isee <- ISEE_residual_lasso(X, true_labels, K)

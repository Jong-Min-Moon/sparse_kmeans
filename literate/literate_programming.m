...
% 
% 
function is_stuck = detect_rand_1(obj_val_vec,   window_size)
% detect_relative_change - Checks whether the last two valid objective values
% show insufficient relative improvement.
%
% Inputs:
...
%   is_stuck    - Logical flag: true if relative change < min_delta
    ...
    % Need at least two valid values to compute relative change
    if numel(valid_vals) < max(2, window_size)
        is_stuck = false;
        return;
    end
    % Compute relative change
  
    curr_val = valid_vals(end-window_size+1 : end);
...
%% 
%% UCB type algorithm
%% sdp_kmeans_iter_knowncov_average
% @export
classdef sdp_kmeans_iter_knowncov_average < handle
    properties
        X           % Data matrix (d x n)
        K           % Number of clusters
        n           % Number of data points
    ...
    methods
        function obj = sdp_kmeans_iter_knowncov_average(X, K)
            obj.X = X;
            obj.K = K;
            obj.n = size(X, 2);
            obj.p = size(X, 1);
            obj.n_iter = NaN;
            obj.delta = repelem(0,obj.p);
        ...
        function cluster_est_now = fit_predict(obj, n_iter)     
            ...
            for iter = 1:n_iter
                ...
                if iter ==1
                    ...
                    obj.delta = obj.delta*( (iter-1) / iter) + abs_diff/iter;
                end
                cutoff_now = obj.cutoff;
                thresholder_vec = obj.delta > cutoff_now;
                n_selected_features = sum(thresholder_vec);
                fprintf("%i entries survived \n\n",n_selected_features)
                while n_selected_features==0 & cutoff_now>1/10
                    cutoff_now = cutoff_now*0.8;
                    thresholder_vec = obj.delta  > obj.cutoff;
                    n_selected_features = sum(thresholder_vec);
                    fprintf("%i entries survived \n\n",n_selected_features)
                end
                x_sub_now = obj.X(thresholder_vec,:);
...
% ISEE_kmeans_clean - Runs iterative clustering with early stopping and logs results to SQLite DB
    ...
    while (~is_stop) && (iter < n_iter)
        iter= iter+1;
        [cluster_estimate_new, s_hat, obj_sdp(iter), obj_lik(iter)] = ISEE_kmeans_clean_onestep(x, k, cluster_estimate_now, is_parallel);
       %%%%%%%%%%%%%%%% simul part starts
        TP = sum(s_hat(1:10));
        FP = sum(s_hat) - TP;
        FN = 10 - TP;
acc = get_bicluster_accuracy(cluster_estimate_new, cluster_true);  % define this if needed
fprintf('Iteration %d | SDP obj: %.4f | Likelihood obj: %.4f | TP: %d | FP: %d | FN: %d | Acc: %.4f\n', ...
    iter, obj_sdp(iter), obj_lik(iter), TP, FP, FN, acc);
    ...
    %%%%%%%%%%%%% simul part ends
       % Early stopping condition
       rand_score = RandIndex(cluster_estimate_now, cluster_estimate_new)
       rand_vec(iter) = rand_score;
        is_stop = decide_stop_rand(rand_vec, loop_detect_start, window_size, min_delta);
        cluster_estimate_now = cluster_estimate_new;
        if is_stop
            break;
        end
    end

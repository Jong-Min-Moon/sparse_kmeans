...
ssh "${USERNAME}@${HOSTNAME}" "mkdir -p ${REMOTE_BASE}/simulations/sim_v3_oracle_isee && mkdir -p ${REMOTE_BASE}/code_r"

# 2. Transfer Simulation Files
echo -e "${CYAN}Transferring simulation files...${NC}"
scp "${SIM_DIR}/run_sim_v3.R" "${SIM_DIR}/submit_v3.slurm" "${USERNAME}@${HOSTNAME}:${REMOTE_BASE}/simulations/sim_v3_oracle_isee/"
...
SUBMIT_CMD="cd ${REMOTE_BASE}/simulations/sim_v3_oracle_isee && \
...
cd ${REMOTE_BASE}/simulations/sim_v3_oracle_isee && \

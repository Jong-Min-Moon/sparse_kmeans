<#
.SYNOPSIS
    Retrieves Simulation 02 results from the HPC cluster.

.DESCRIPTION
    Uses 'scp' to copy all results from the remote server to the local 'simulations/sim02_unknown_cov_greedy' directory.
...
.PARAMETER RemoteBase
    ...
    Results are expected in <RemoteBase>/simulations/sim02_unknown_cov_greedy/results/.
    default: ~/sparse_kmeans_project

.PARAMETER LocalDir
    The local directory to download results to.
    default: d:\GitHub\sparse_kmeans\simulations\sim02_unknown_cov_greedy
...
param(
    ...
    [string]$LocalDir = "d:\GitHub\sparse_kmeans\simulations\sim02_unknown_cov_greedy"
)

$ErrorActionPreference = "Stop"

$RemoteSimDir = "${RemoteBase}/simulations/sim02_unknown_cov_greedy"

# ---------------------------------------------------------
# Single Simulation Run: Thompson Sampling v3.1 (First-Iter Oracle ISEE)
# sim10 variant: ISEE uses true cluster labels at iteration 1 only.
# From iteration 2 onward, reverts to estimated cluster labels.
# Seeds and all other settings identical to sim_v3_1_oracle_isee.
# ---------------------------------------------------------
library(methods)
library(MASS)
library(mclust)
...
if (length(args) > 0) {
    for (i in seq(1, length(args), by = 2)) {
        ...
        if (args[i] == "--p_val") p_val_target <- as.numeric(args[i + 1])
        if (args[i] == "--perms") n_perms <- as.integer(args[i + 1])
        if (args[i] == "--C") C <- as.numeric(args[i + 1])
    }
}

# Source Code
source("../../code_r/sparse_symmetric_data_generator.R")
source("../../code_r/block_coordinate_optim_thompson_unknowncov_v3_1_first_iter_oracle.R") # sim10 variant
source("../../code_r/block_coordinate_optim_permutation.R")
...
# ---------------------------------------------------------
# Data Generation Parameters (IDENTICAL to sim_v3_1_oracle_isee)
# ---------------------------------------------------------
p <- 400
n <- 500
K <- 2
...
cat(sprintf("--- Simulation Run sim10 First-Iter Oracle ISEE (Job ID: %d) ---\n", job_id))
cat(sprintf("Params: p=%d, n=%d, sep=%.1f, rho=%.2f\n", p, n, separation, rho))
cat(sprintf("Method: Thompson Sampling v3.1 First-Iter Oracle ISEE (P-val=%.4f, C=%.2f)\n", p_val_target, C))
...
# 2. Generate Data (SAME seed as sim_v3_1_oracle_isee)
...
# Run Algorithm
# ---------------------------------------------------------
cat("Running block_coordinate_optim_thompson_unknowncov_v3_1_first_iter_oracle...\n")
...
start_time <- Sys.time()
res <- block_coordinate_optim_thompson_unknowncov_v3_1_first_iter_oracle(
    X = X,
    K = 2,
    n_iter = 1000,
    C = C,
    n_perms = n_perms,
    p_val_threshold = p_val_target,
    max_iter_sdp = 4000,
...
saveRDS(list(
    ...
    fp = fp,
    job_id = job_id,
    params = list(p = p, n = n, rho = rho, sep = separation, p_val = p_val_target, n_perms = n_perms, C = C)

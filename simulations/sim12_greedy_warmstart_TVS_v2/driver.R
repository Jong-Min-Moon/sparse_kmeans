# ---------------------------------------------------------
# Single Simulation Run: sim12_greedy_warmstart_TVS_v2
# ---------------------------------------------------------
library(methods)
library(MASS)
library(mclust)
...
cat(sprintf("--- Simulation Run sim12_greedy_warmstart_TVS_v2 (Job ID: %d) ---\n", job_id))
...
res <- block_coordinate_optim_warmstart_tvs(
    X = X,
    K = 2,
    n_iter_greedy = n_iter_greedy,
    n_iter_tvs = n_iter_tvs,
    n_perms_greedy = n_perms,
    fdr_target_greedy = fdr,
    C = 0.5,

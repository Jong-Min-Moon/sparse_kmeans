...
for (noise in noise_types) {
    ...
    # ------- Persist ---------------------------------------------------
    saveRDS(df,         file = sprintf("aggregated_unknowncov_%s.rds", noise))
    saveRDS(summary_df, file = sprintf("summary_unknowncov_%s.rds",    noise))
    write.csv(summary_df,
              file      = sprintf("summary_unknowncov_%s.csv", noise),
              row.names = FALSE)

    cat(sprintf("\n[%s] Saved: aggregated_unknowncov_%s.rds, summary_unknowncov_%s.{rds,csv}\n\n",
                noise, noise, noise))
}

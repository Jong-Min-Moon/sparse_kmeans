...
param(
    ...
    [int[]]$Dimensions = @(100, 200, 300, 400),
...
$SimDir = "d:\GitHub\sparse_kmeans\simulations\production\unknowncov_n500_chain_thompson"
$CodeDir = "d:\GitHub\sparse_kmeans\code_r"

# 1. Create Remote Directories
Write-Host "Creating remote directories..." -ForegroundColor Cyan
ssh "${Username}@${Hostname}" "mkdir -p ${RemoteBase}/simulations/production/unknowncov_n500_chain_thompson && mkdir -p ${RemoteBase}/code_r"

# 2. Transfer Simulation Files (Driver, Submit Script)
Write-Host "Transferring simulation files..." -ForegroundColor Cyan
scp "${SimDir}\driver.R" "${SimDir}\submit.sh" "${Username}@${Hostname}:${RemoteBase}/simulations/production/unknowncov_n500_chain_thompson/"
...
$prepCmd = "cd ${RemoteBase}/simulations/production/unknowncov_n500_chain_thompson && rm -rf logs results_raw results_aggregated *.out *.err && mkdir -p logs results_raw results_aggregated && dos2unix *.sh *.R 2>/dev/null && chmod +x *.sh"
...
foreach ($sep in $Separations) {
    foreach ($p in $Dimensions) {
        ...
        $sbatchCmd = "cd ${RemoteBase}/simulations/production/unknowncov_n500_chain_thompson && sbatch --output=logs/sim_id%a_sep${sep}_p${p}.out --error=logs/sim_id%a_sep${sep}_p${p}.err --export=ALL,SEP=$sep,P=$p,NOISE=$Noise submit.sh"

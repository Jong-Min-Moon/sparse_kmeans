# ------------------------------------------------------------------------------
# Script: aggregate_results.R
# Purpose: Compiles, merges, and validates the output generated across isolated 
#          HPC computational nodes running `thompson_unknowncov_naive`. 
...
summary_file <- "summary_thompson_unknowncov_naive.csv"

if (!dir.exists(output_dir)) {
    stop("Output directory not found: ", output_dir, "\nRun retrieve_thompson_unknowncov_naive.ps1 using Powershell first.")
}
...
cat("\n=== Thompson Unknown-Covariance Naive Performance Summary Evaluator ===\n")
print(summary_stats)

# Export monolithic RDS database targeting any final customized post-mortem tracking needs
saveRDS(all_results, "all_results_thompson_unknowncov_naive.rds")

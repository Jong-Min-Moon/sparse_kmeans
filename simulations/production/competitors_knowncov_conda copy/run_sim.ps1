...
switch ($Sim.ToLower()) {
    "laplace"  { Invoke-Sim "$Root\sim_laplace_knowncov.R"  "Laplace" }
    "gaussian" { Invoke-Sim "$Root\sim_gaussian_knowncov.R" "Gaussian" }
    default    {
        Invoke-Sim "$Root\sim_laplace_knowncov.R"  "Laplace"
        Invoke-Sim "$Root\sim_gaussian_knowncov.R" "Gaussian"
    }
}

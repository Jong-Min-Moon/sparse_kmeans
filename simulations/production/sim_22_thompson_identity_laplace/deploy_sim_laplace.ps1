...
$SimDir = "d:\GitHub\sparse_kmeans\simulations\production\sim_22_thompson_identity_laplace"
$CodeDir = "d:\GitHub\sparse_kmeans\code_r"

# 1. Create Remote Directories
Write-Host "Creating remote directories..." -ForegroundColor Cyan
ssh "${Username}@${Hostname}" "mkdir -p ${RemoteBase}/simulations/production/sim_22_thompson_identity_laplace && mkdir -p ${RemoteBase}/code_r"

# 2. Transfer Simulation Files (Driver, Submit Script)
Write-Host "Transferring simulation files..." -ForegroundColor Cyan
scp "${SimDir}\driver.R" "${SimDir}\submit.sh" "${Username}@${Hostname}:${RemoteBase}/simulations/production/sim_22_thompson_identity_laplace/"
...
$prepCmd = "cd ${RemoteBase}/simulations/production/sim_22_thompson_identity_laplace && rm -rf logs results_raw results_aggregated *.out *.err && mkdir -p logs results_raw results_aggregated && dos2unix *.sh *.R 2>/dev/null && chmod +x *.sh"
...
foreach ($sep in $Separations) {
    foreach ($p in $Dimensions) {
        ...
        $sbatchCmd = "cd ${RemoteBase}/simulations/production/sim_22_thompson_identity_laplace && sbatch --output=logs/sim_id%a_sep${sep}_p${p}.out --error=logs/sim_id%a_sep${sep}_p${p}.err --export=ALL,SEP=$sep,P=$p submit.sh"

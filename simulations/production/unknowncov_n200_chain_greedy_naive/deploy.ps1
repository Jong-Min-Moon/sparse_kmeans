<#
.SYNOPSIS
    Deploys unknowncov_n200_chain_greedy_naive simulation to the HPC cluster.
    Data setting:  chain-graph covariance (rho=0.45, sparsity=2) — from competitors_unknowncov.
    Algorithm:     cluster_greedy — from knowncov_greedy.
...
.PARAMETER Separations
    Array of separation values. Default: @(6)
...
$SimName = "unknowncov_n200_chain_greedy_naive"
...
scp "${SimDir}\driver_gaussian.R" "${SimDir}\driver_laplace.R" "${SimDir}\submit.sh" `
...
$prepCmd = "cd ${RemoteBase}/simulations/production/${SimName} && rm -rf logs results_raw *.out *.err && mkdir -p logs results_raw && dos2unix *.sh *.R 2>/dev/null; chmod +x *.sh"
...
foreach ($sep in $Separations) {
    foreach ($p in $Dimensions) {
        ...
        # Brief pause to avoid SSH rate-limiting

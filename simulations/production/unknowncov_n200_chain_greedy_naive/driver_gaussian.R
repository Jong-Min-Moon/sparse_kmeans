# ==============================================================================
# driver_gaussian.R
# HPC Array Script: cluster_greedy under Gaussian Noise, Chain-Graph Covariance
# ==============================================================================
# Data setting:  chain-graph precision matrix, rho = 0.45, sparsity = 2
#                — identical to competitors_unknowncov (sim_gaussian.R)
...
cat(sprintf(
    "\n--- unknowncov_n200_chain_greedy_naive | Gaussian | Job %d, p=%d, sep=%.1f ---\n",
    job_id, p, sep
))
...
if (!is.null(res)) {
    ...
    cat(sprintf("Accuracy: %.4f | Selected: %d (TP=%d FP=%d) | Recall=%.4f Precision=%.4f\n",

# ------------------------------------------------------------------
# sim_laplace_ar1_conda.R
# scvxclustr Evaluation under Laplace Noise (AR1 Setting)
# ------------------------------------------------------------------
library(foreach)
library(doParallel)
...
log_dir <- "logs"
res_dir <- "results_ar1/laplace"

if (!dir.exists(log_dir)) dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)
if (!dir.exists(res_dir)) dir.create(res_dir, recursive = TRUE, showWarnings = FALSE)

log_file <- file.path(log_dir, "sim_laplace_ar1_conda.log")
if (!file.exists(log_file)) file.create(log_file)

cat(sprintf("Starting Laplace AR1 Simulation for scvxclustr. Results will be saved in %s/\n", res_dir))
log_progress(log_file, sprintf(
    "\n==========================================\n--- Laplace AR1 Started at %s ---\n==========================================\n",
    Sys.time()
))
...
log_progress(log_file, sprintf(
    "\n==========================================\n--- Laplace AR1 Simulation Completed at %s (Runtime: %.2f mins) ---\n==========================================\n",

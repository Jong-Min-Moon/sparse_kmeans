# ------------------------------------------------------------------
# sim_laplace_knowncov.R
# Competitor Evaluation under Laplace Noise (Known Covariance)
# ------------------------------------------------------------------
library(foreach)
library(doParallel)
...
# Source local utilities for Conda-isolated reproducibility
source("sim_utils.R")
...
p_seq <- seq(3000, 12000, by = 3000)
noise_type <- "Laplace"

# Setup Directories & Logging
log_dir <- "logs"
res_dir <- "results/laplace"

if (!dir.exists(log_dir)) dir.create(log_dir, recursive = TRUE, showWarnings = FALSE)
if (!dir.exists(res_dir)) dir.create(res_dir, recursive = TRUE, showWarnings = FALSE)

log_file <- file.path(log_dir, "sim_laplace_knowncov.log")
if (!file.exists(log_file)) file.create(log_file)

cat(sprintf("Starting Laplace Known-Covariance Simulation. Results will be saved in %s/\n", res_dir))
log_progress(log_file, sprintf("\n==========================================\n--- Laplace Known-Covariance Started at %s ---\n==========================================\n", Sys.time()))
...
for (p in p_seq) {
    ...
    results <- foreach(
        ...
        # 6. Logging
        msg <- sprintf(
            "[%s] Rep %d, p = %d: Witten = %.3f, Arias = %.3f, IF-PCA = %.3f, BClust = %.3f, SCVX = %.3f\n",
            format(Sys.time(), "%Y-%m-%d %H:%M:%S"), job_id, p,
            sim_out$res_df$accuracy_witten, sim_out$res_df$accuracy_arias, 
            sim_out$res_df$accuracy_ifpca, sim_out$res_df$accuracy_bclust,
            sim_out$res_df$accuracy_scvx
        )
        log_progress(log_file, msg)
...
log_progress(log_file, sprintf("\n==========================================\n--- Laplace Known-Cov Simulation Completed at %s (Runtime: %.2f mins) ---\n==========================================\n", Sys.time(), overall_runtime))

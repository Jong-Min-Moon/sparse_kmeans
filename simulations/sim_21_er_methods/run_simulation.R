# ------------------------------------------------------------------
# run_simulation.R
# Main driver script for sim_21 (Evaluating methods from sim_17 on ER data)
# ------------------------------------------------------------------
library(foreach)
library(doParallel)
...
K <- 2
s <- 10
separations <- list(NULL) # adjust as needed based on sim20 setting, using 2 as placeholder or pass in NULL for auto
n_runs <- 100 # based on typical sim_20 runs
...
cat(sprintf("Starting Unified Simulation (sim_21) using %d cores...\n", n_cores))
...
for (sep in separations) {
    if (is.null(sep)) sep_val <- "NULL" else sep_val <- sep
    cat(sprintf("\n--- Starting jobs for separation = %s ---\n", sep_val))
    ...
    results <- foreach(
        ...
        data_res <- generate_erdos_renyi_data(n = n, p = p, separation = sep, s = s)
        X <- data_res$X
        ...
        log_msg <- sprintf(
            "Rep %d, sep = %s: Witten [feat = %s, acc = %.3f], Arias [feat = %s, acc = %.3f], IF-PCA [feat = %s, acc = %.3f]\n",
            job_id, sep_val,
        ...
        saveRDS(res_df, file = sprintf("results/sim_id%d_sep%s.rds", job_id, sep_val))

        return(res_df)
    }
    
    # Optionally Save aggregated for this sep
    saveRDS(results, file = sprintf("results_aggregated_sep%s.rds", sep_val))
}

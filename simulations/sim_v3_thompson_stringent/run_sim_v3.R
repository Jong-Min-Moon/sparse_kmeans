# ---------------------------------------------------------
# Single Simulation Run: Thompson Sampling v3 (Oracle ISEE, Stringent P-val)
# ---------------------------------------------------------
library(methods)
library(MASS)
library(mclust)
...
cat(sprintf("Method: Thompson Sampling v3 (Oracle ISEE Init, P-val=%.4f)\n", p_val_target))
...
saveRDS(list(
    ...
    fp = fp,
    job_id = job_id,
    params = list(p = p, n = n, rho = rho, sep = separation, p_val = p_val_target, n_perms = n_perms)

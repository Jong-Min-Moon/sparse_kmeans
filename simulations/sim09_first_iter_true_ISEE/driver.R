# ---------------------------------------------------------
# Single Simulation Run: Permutation FDR (First-Iter Oracle ISEE)
# MODIFICATION from sim07: At the FIRST iteration only, ISEE uses TRUE cluster
# labels. From iteration 2 onward, reverts to estimated labels (identical to sim07).
# Seed and data settings are identical to sim07 for direct comparison.
# ---------------------------------------------------------
library(methods)
library(MASS)
library(mclust)
...
source("../../code_r/block_coordinate_optim_permutation_first_iter_oracle_ISEE.R") # Hybrid variant
...
# ---------------------------------------------------------
# Data Generation Parameters (IDENTICAL to sim07 and sim08)
# ---------------------------------------------------------
p <- 400
n <- 500
K <- 2
...
cat(sprintf("--- Simulation Run sim09 First-Iter Oracle ISEE (Job ID: %d) ---\n", job_id))
cat(sprintf("Params: p=%d, n=%d, sep=%.1f, rho=%.2f\n", p, n, separation, rho))
cat(sprintf("Method: Permutation FDR (Target=%.2f, Perms=%d) + FIRST-ITER ORACLE ISEE\n", fdr_target, n_perms))
...
# 2. Generate Data (SAME seed as sim07/sim08)
...
# Run Algorithm
# ---------------------------------------------------------
cat("Running block_coordinate_optim_permutation_first_iter_oracle_ISEE...\n")
...
start_time <- Sys.time()
res <- block_coordinate_optim_permutation_first_iter_oracle_ISEE(
    X = X,
    K = 2,
    n_iter = 100,
    n_perms = n_perms,
    ...
    stable_iter = 5,
    true_labels = true_labels # passed for oracle iter-1 switch AND accuracy logging
)
end_time <- Sys.time()
...
if (length(support) > 0) cat(sprintf("Recall:    %.2f\n", tp / length(support)))
if (length(selected_indices) > 0) cat(sprintf("Precision: %.2f\n", tp / length(selected_indices)))

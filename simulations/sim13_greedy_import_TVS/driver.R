# ---------------------------------------------------------
# Single Simulation Run: sim11_greedy_warmstart_TVS
# ---------------------------------------------------------
library(methods)
library(MASS)
library(mclust)
...
if (length(args) > 0) {
    for (i in seq(1, length(args), by = 2)) {
        ...
        if (args[i] == "--p_val") p_val <- as.numeric(args[i + 1])
        if (args[i] == "--fdr") fdr <- as.numeric(args[i + 1])
        if (args[i] == "--n_iter_greedy") n_iter_greedy <- as.integer(args[i + 1])
        if (args[i] == "--n_iter_tvs") n_iter_tvs <- as.integer(args[i + 1])
    }
}
...
source("../../code_r/block_coordinate_optim_import_tvs.R")
...
cat(sprintf("--- Simulation Run sim11_greedy_warmstart_TVS (Job ID: %d) ---\n", job_id))
cat(sprintf("Params: p=%d, n=%d, sep=%.1f, rho=%.2f\n", p, n, separation, rho))
cat(sprintf("Method: Greedy Warmstart TVS (p_value=%.2f, Perms=%d)\n", p_val, n_perms))
...
# ---------------------------------------------------------
# Run Algorithm (Warmstart TVS)
# ---------------------------------------------------------
cat("Running block_coordinate_optim_warmstart_tvs...\n")
...
res <- block_coordinate_optim_import_tvs(
    X = X,
    K = 2,
    sim_id = job_id,
    fdr_level = fdr,
    n_iter_tvs = n_iter_tvs,
    C = 0.5,
...
# Calculate true overlap at iteration 100
overlap_at_100 <- length(intersect(res$greedy_s_hat_100, support))
cat(sprintf("Overlap at iteration 100: %d / %d\n", overlap_at_100, length(support)))
...
saveRDS(list(
    ...
    job_id = job_id,
    params = list(p = p, n = n, rho = rho, sep = separation, fdr = fdr, n_perms = n_perms)

...
# Define Local Paths
$SimDir = "d:\GitHub\sparse_kmeans\simulations\sim_18"
$CodeDir = "d:\GitHub\sparse_kmeans\code_r"

# 1. Create Remote Directories
Write-Host "Creating remote directories..." -ForegroundColor Cyan
ssh "${Username}@${Hostname}" "mkdir -p ${RemoteBase}/simulations/sim_18 && mkdir -p ${RemoteBase}/code_r"

# 2. Transfer Simulation Files (Driver, Submit Script)
Write-Host "Transferring simulation files..." -ForegroundColor Cyan
scp "${SimDir}\driver.R" "${SimDir}\submit.sh" "${Username}@${Hostname}:${RemoteBase}/simulations/sim_18/"
...
$prepCmd = "cd ${RemoteBase}/simulations/sim_18 && rm -rf logs results_raw results_aggregated *.out *.err && mkdir -p logs results_raw results_aggregated && dos2unix *.sh *.R 2>/dev/null && chmod +x *.sh"
...
foreach ($sep in $Separations) {
    foreach ($pval in $Pvals) {
        ...
        $sbatchCmd = "cd ${RemoteBase}/simulations/sim_18 && sbatch --output=logs/sim_id%a_sep${sep}_pval${pvalStr}.out --error=logs/sim_id%a_sep${sep}_pval${pvalStr}.err --export=ALL,SEP=$sep,PVAL=$pval submit.sh"

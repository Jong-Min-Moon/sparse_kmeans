# ---------------------------------------------------------
# Single Simulation Run: Permutation FDR (Oracle ISEE)
# MODIFICATION from sim07: ISEE step uses TRUE cluster labels instead of
# estimated labels. All other settings (data gen, seed, FDR target) are
...
source("../../code_r/block_coordinate_optim_permutation_oracle_ISEE.R") # Oracle variant
...
# ---------------------------------------------------------
# Data Generation Parameters (IDENTICAL to sim07)
# ---------------------------------------------------------
p <- 400
n <- 500
K <- 2
...
cat(sprintf("--- Simulation Run sim08 Oracle ISEE (Job ID: %d) ---\n", job_id))
cat(sprintf("Params: p=%d, n=%d, sep=%.1f, rho=%.2f\n", p, n, separation, rho))
cat(sprintf("Method: Permutation FDR (Target=%.2f, Perms=%d) + ORACLE ISEE\n", fdr_target, n_perms))
...
# 2. Generate Data (SAME seed as sim07 for direct comparison)
...
# ---------------------------------------------------------
# Run Algorithm (Oracle ISEE Variant)
# ---------------------------------------------------------
cat("Running block_coordinate_optim_permutation_oracle_ISEE...\n")
...
start_time <- Sys.time()
res <- block_coordinate_optim_permutation_oracle_ISEE(
    X = X,
    K = 2,
    n_iter = 100,
    n_perms = n_perms,
    ...
    stable_iter = 5,
    true_labels = true_labels # passed for both oracle ISEE AND accuracy logging
)
end_time <- Sys.time()

...
ssh "${Username}@${Hostname}" "mkdir -p ${RemoteBase}/simulations/${SimName} && mkdir -p ${RemoteBase}/simulations/competitors_unknowncov && mkdir -p ${RemoteBase}/code_r"
...
scp "d:\GitHub\sparse_kmeans\simulations\competitors_unknowncov\sim_utils.R" `
    "${Username}@${Hostname}:${RemoteBase}/simulations/competitors_unknowncov/"

# 5. Prepare HPC environment
Write-Host "Preparing HPC environment..." -ForegroundColor Cyan
$prepCmd = "cd ${RemoteBase}/simulations/${SimName} && rm -rf logs results_raw results_aggregated *.out *.err && mkdir -p logs results_raw && dos2unix *.sh *.R 2>/dev/null; chmod +x *.sh"
...
foreach ($sep in $Separations) {
    foreach ($p in $Dimensions) {
        ...
        $sbatchCmd = "cd ${RemoteBase}/simulations/${SimName} && sbatch --output=${logPattern}.out --error=${logPattern}.err --export=ALL,SEP=$sep,P=$p,NOISE=$Noise,METHODS=$Methods submit.sh"

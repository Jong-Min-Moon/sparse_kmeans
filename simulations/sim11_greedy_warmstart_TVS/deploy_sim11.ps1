...
# Define Local Paths
$SimDir = "d:\GitHub\sparse_kmeans\simulations\sim11_greedy_warmstart_TVS"
$CodeDir = "d:\GitHub\sparse_kmeans\code_r"

# 1. Create Remote Directories
Write-Host "Creating remote directories..." -ForegroundColor Cyan
ssh "${Username}@${Hostname}" "mkdir -p ${RemoteBase}/simulations/sim11_greedy_warmstart_TVS && mkdir -p ${RemoteBase}/code_r"

# 2. Transfer Simulation Files (Driver, Submit Script)
Write-Host "Transferring simulation files..." -ForegroundColor Cyan
scp "${SimDir}\driver.R" "${SimDir}\submit.sh" "${Username}@${Hostname}:${RemoteBase}/simulations/sim11_greedy_warmstart_TVS/"
...
$submitCmd = "cd ${RemoteBase}/simulations/sim11_greedy_warmstart_TVS && rm -rf logs results *.out *.err && dos2unix *.sh *.R && chmod +x *.sh && sbatch submit.sh"

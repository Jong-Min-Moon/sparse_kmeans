# ---------------------------------------------------------
# Single Simulation Run: Thompson Sampling v4 (SAM FDR)
# ---------------------------------------------------------
library(methods)
library(MASS)
library(mclust)
...
# Source Code (Adjusted paths for simulations/sim_v4_thompson_fdr/)
source("../../code_r/sparse_symmetric_data_generator.R")
source("../../code_r/block_coordinate_optim_thompson_unknowncov_v4.R")
...
cat(sprintf("Method: Thompson Sampling v4 (SAM FDR)\n"))
...
# ---------------------------------------------------------
# Run Algorithm (Thompson Sampling v4)
# ---------------------------------------------------------
cat("Running block_coordinate_optim_thompson_unknowncov_v4...\n")

start_time <- Sys.time()
res <- block_coordinate_optim_thompson_unknowncov_v4(
    X = X,
    K = 2,
    n_iter = 100,
    C = 0.5,
    n_perms = n_perms,
    fdr_level = fdr_target,
    max_iter_sdp = 4000
)
end_time <- Sys.time()

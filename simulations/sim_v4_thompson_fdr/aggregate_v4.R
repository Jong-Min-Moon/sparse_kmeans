# Aggregate Simulation v4 Results and Plot Trajectories
...
cat(sprintf("Found %d v4 result files. Aggregating...\n", length(all_files)))
...
mean_traj_v4 <- colMeans(traj_mat, na.rm = TRUE)
...
cat("\n=== Simulation v4 SAM FDR Summary ===\n")
print(summary_stats)
saveRDS(all_results, "all_results_sim_v4.rds")

# --- 3. Gather v2 Results (if available) for Trajectory Plot ---
mean_traj_v2 <- rep(NA, length(mean_traj_v4))
if (dir.exists(v2_output_dir)) {
    ...
    cat("Warning: v2 baseline results not found at", v2_output_dir, "\nPlotting v4 trajectory only...\n")
}

# --- 4. Plot Trajectories ---
traj_len <- length(mean_traj_v4)
plot_data <- data.frame(
    Iteration = rep(1:traj_len, 2),
    Objective = c(mean_traj_v4, mean_traj_v2),
    Model = rep(c("v4 (SAM FDR)", "v2 (Standard)"), each = traj_len)
)
...
p <- ggplot(plot_data, aes(x = Iteration, y = Objective, color = Model, linetype = Model)) +
    ...
    theme_minimal() +
    labs(
        title = "Convergence Trajectory: Oracle (v4) vs Standard (v2)",
        x = "Iteration",

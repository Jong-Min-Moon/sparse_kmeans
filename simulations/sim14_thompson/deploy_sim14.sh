...
ssh "${USERNAME}@${HOSTNAME}" "mkdir -p ${REMOTE_BASE}/simulations/sim14_thompson && mkdir -p ${REMOTE_BASE}/code_r"

# 2. Transfer Simulation Files (Driver, Submit Script)
echo -e "${CYAN}Transferring simulation files...${NC}"
scp "${SIM_DIR}/driver.R" "${SIM_DIR}/submit.sh" "${USERNAME}@${HOSTNAME}:${REMOTE_BASE}/simulations/sim14_thompson/"
...
PREP_CMD="cd ${REMOTE_BASE}/simulations/sim14_thompson && \
...
for sep in "${SEPARATIONS[@]}"; do
    for pval in "${PVALS[@]}"; do
        ...
        SBATCH_CMD="cd ${REMOTE_BASE}/simulations/sim14_thompson && \

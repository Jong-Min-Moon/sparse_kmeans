function [cluster_acc, abs_diff, thres] = Nov_21_2023_one_threshold_for_two_group_fixed_NaN(x, sigma, K, p, n_iter, rounding, n, cluster_true)     
%data generation
abs_diff = zeros([n_iter, p]);
thres = zeros(n_iter);
x_now = x;
A_now = (x_now * x_now')/ n;
...
for iter = 1:n_iter
    ...
    thres(iter) = sqrt(2 * log(p) ) * sqrt(sigma^2*(1/max(1,n_g1_now) + 1/max(1,n_g2_now)));
    
    fprintf("absolute difference:")
    abs_diff(iter,:) = abs(x_bar_g1 - x_bar_g2);
    abs_diff(iter,:)
    fprintf("\n")

    fprintf("threshold = %f\n", thres(iter))
    thresholder_vec = abs_diff(iter,:) > thres(iter);

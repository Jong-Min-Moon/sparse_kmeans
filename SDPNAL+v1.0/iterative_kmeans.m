...
for iter = 1:n_iter
    ...
    abs_diff = abs(x_bar_g1 - x_bar_g2);
    n_factor = (n_g1_now*n_g2_now)/(n_g1_now+n_g2_now);
    fprintf("normalized difference:")
    abs_diff_normalized = abs_diff/sigma*(sqrt(n_factor))
   % thresholder_vec = abs(x_bar_g1 - x_bar_g2) > c;
    thresholder_vec = abs_diff_normalized > c;
